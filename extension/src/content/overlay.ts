...
const STYLES = `
  ...
  *{box-sizing:border-box}.panel{position:relative;display:none;pointer-events:auto;text-align:center;color:#fff;filter:drop-shadow(0 3px 14px rgba(0,0,0,.72))}.panel.visible{display:block}
  .caption{display:none;position:relative;flex-direction:column;align-items:center;gap:5px;max-width:100%;padding:10px 16px 11px;border-radius:12px;background:rgba(10,11,15,.76);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);cursor:grab;touch-action:none}.caption.visible{display:inline-flex}.caption.dragging{cursor:grabbing}
